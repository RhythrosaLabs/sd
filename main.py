...
with tabs[0]:
    ...
    with st.expander("Canvas Options", expanded=True):
    ...
    st.subheader("🖌️ Generate and Modify Images")
    with st.expander("Generation Settings", expanded=True):
        generation_type = st.selectbox("Select Generation Type", ["Text-to-Image", "Image-to-Image", "Image Effects"], key="generation_type")
        ...
        # Common parameters
        prompt = st.text_area("Prompt", key="prompt", help="Describe the image you want to generate.")
        negative_prompt = st.text_area("Negative Prompt", key="negative_prompt", help="Describe what you don't want in the image.")
        aspect_ratio = st.selectbox("Aspect Ratio", ["1:1", "16:9", "21:9", "2:3", "3:2", "4:5", "5:4", "9:16", "9:21"], key="aspect_ratio")
        seed = st.number_input("Seed (0 for random)", min_value=0, max_value=4294967294, value=0, key="seed")
        output_format = st.selectbox("Output Format", ["png", "jpeg", "webp"], key="output_format")
        ...
        if model_type in ["Stable Image Ultra", "Stable Image Core"]:
            ...
            if model_type == "Stable Image Ultra":
                cfg_scale = st.slider("CFG Scale", min_value=0.0, max_value=35.0, value=7.0, key="cfg_scale")
            else:
            ...
            steps = st.number_input("Steps", min_value=1, max_value=150, value=50, key="steps")
            sampler = st.selectbox("Sampler", ["DDIM", "DDPM", "K_DPMPP_2M", "K_DPMPP_2S_ANCESTRAL", "K_DPM_2", "K_DPM_2_ANCESTRAL", "K_EULER", "K_EULER_ANCESTRAL", "K_HEUN", "K_LMS"], key="sampler")
            cfg_scale = st.slider("CFG Scale", min_value=0.0, max_value=35.0, value=7.0, key="cfg_scale")
            samples = st.number_input("Samples", min_value=1, max_value=10, value=1, key="samples")
        ...
        if generation_type == "Text-to-Image":
            ...
            if st.session_state['current_image'] is not None:
                if effect_type == "Upscale":
                    ...
                    creativity = st.slider("Creativity", min_value=0.0, max_value=1.0, value=0.5, key="creativity")
            ...
            st.error("Invalid Generation Type Selected.")
...
with tabs[1]:
    ...
    with video_subtabs[0]:
        ...
        st.warning("Text-to-Animation feature is currently under development.")
